"""Database schema and PostgreSQL maintenance helpers."""
...
from python.database.base import (
    BASE_DIR,
    Base,
...
def _ensure_columns_and_indexes() -> None:
    with engine.begin() as con:
        ...
        for column_name in ("machine_type", "machine_id"):
            con.exec_driver_sql(
                "ALTER TABLE public.problem_matches "
                f"ADD COLUMN IF NOT EXISTS {column_name} TEXT DEFAULT ''"
            )
            con.exec_driver_sql(
                "UPDATE public.problem_matches "
                f"SET {column_name} = '' "
        ...
        con.exec_driver_sql(
            "ALTER TABLE public.problem_matches "
            "DROP CONSTRAINT IF EXISTS uq_problem_match_machine_problem"
        )
        con.exec_driver_sql(
            "ALTER TABLE public.problem_matches "
            "DROP CONSTRAINT IF EXISTS uq_problem_match_scope"
        )
        con.exec_driver_sql(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS uq_problem_match_scope_idx
            ON public.problem_matches (machine, machine_type, machine_id, problem)
            """
        )
        ...
        for index_sql in (
            ...
            "CREATE INDEX IF NOT EXISTS idx_problem_matches_machine_problem ON problem_matches(machine, problem)",
            "CREATE INDEX IF NOT EXISTS idx_problem_matches_machine_type ON problem_matches(machine, machine_type)",
            "CREATE INDEX IF NOT EXISTS idx_problem_matches_machine_id ON problem_matches(machine_id)",
        ):
    ...
    table_specs = (
        ...
        },
        {
            ...
            SELECT
                ...
                coalesce(nullif(pm.machine_type, ''), '-') AS "Machine Type",
                coalesce(nullif(pm.machine_id, ''), '-') AS "Machine ID",
            ...
            ORDER BY pm.machine ASC, pm.machine_type ASC, pm.machine_id ASC, pm.problem ASC, pm.class_name ASC, pm.id ASC
            """,
        },
    )
...
def run_db_maintenance(
    enable_display_tables: bool | None = None,
    drop_display_artifacts: bool = False,
) -> None:
    _ensure_postgres_history_view()

    if enable_display_tables is None:
        enable_display_tables = ENABLE_POSTGRES_DISPLAY_TABLES

    if enable_display_tables:
        _ensure_postgres_history_table()
        _ensure_postgres_manage_users_table()
        _ensure_postgres_add_machine_tables()
    elif drop_display_artifacts:
